...
const buildQueueSummary = () => {
  ...
  if (snapshot.active) {
    lines.push(
      `- 実行中: [${STATUS_LABELS[snapshot.active.status] ?? snapshot.active.status}] \`${snapshot.active.filename}\` (ID: \`${snapshot.active.id}\`)`
    );
  } else {
    lines.push("- 実行中: なし");
  }
  ...
  if (recent) {
    ...
    const finished = recent.finishedAt
      ...
      : "-";
    const exitCode =
      recent.result && recent.result.exitCode !== null
    ...
    lines.push(
      `- 直近履歴: [${status}] \`${recent.filename}\` (終了: ${finished})`
    );
    if (recent.result) {
      lines.push(
        `  Run ID: \`${recent.result.runId}\` / 終了コード: ${exitCode} / タイムアウト: ${timedOut}`
      );
...
const execute = async (
  ...
  const botSection = [
    ...
    `- Bot Runtime: v${packageJson.version}`,
  ];
  ...
  const docsUpdateDefault =
    process.env.CODEX_DOCS_UPDATE_ENABLED ??
    "(env未設定: 設定ファイルの既定値を使用)";

  const healthSection = ["**ヘルスチェック**"];

  const healthSummary = collectHealthIssueSummary(5);

  if (healthSummary.total === 0) {
    healthSection.push("- 問題なし");
  } else {
    healthSection.push(
      `- 合計 ${healthSummary.total} 件のヘルス警告があります。`
    );
...
export const statusCommand: SlashCommandModule = {
  data,
  execute,
};
