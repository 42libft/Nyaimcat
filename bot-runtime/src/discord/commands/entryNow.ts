...
const data = new SlashCommandBuilder()
  .setName("entry-now")
  .setDescription("ESCL 応募を即時に送信します（リトライなし）。")
...
const execute = async (
  ...
  try {
    ...
    logger.error("/entry-now コマンドの実行に失敗しました", {
      message,
      scrimId,
...
export const entryNowCommand: SlashCommandModule = {
  data,
  execute,
  autocomplete: async (interaction: AutocompleteInteraction, context) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      logger.error("/entry-now コマンドのアカウント補完に失敗しました", {
        message,
        userId: interaction.user.id,
      });
      await interaction.respond([]);
