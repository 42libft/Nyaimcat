...
type HelpCategoryKey = (typeof helpCategoryConfigs)[number]["key"];
type HelpCategoryConfig = (typeof helpCategoryConfigs)[number];

const helpCategoryMap = new Map<HelpCategoryKey, HelpCategoryConfig>(
  helpCategoryConfigs.map((config) => [config.key, config])
);
...
type HelpCommandKey = (typeof helpCommandConfigs)[number]["key"];
type HelpCommandConfig = (typeof helpCommandConfigs)[number];

const helpCommandMap = new Map<HelpCommandKey, HelpCommandConfig>(
  helpCommandConfigs.map((config) => [config.key, config])
);

const buildCategoryLines = (config: HelpCategoryConfig) => {
  const lines = [config.title, ""];
  lines.push(...config.lines);
  return lines;
};

const buildCommandLines = (config: HelpCommandConfig) => {
...
const buildDefaultLines = () => {
  ...
  lines.push(
    "",
    "例: `/help command:work.start` で `/work start` の詳しい使い方を確認できます。",
    "詳細な運用手順は `README.md` や `docs/` 配下のドキュメントも併せて参照してください。"
  );
...
const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      ...
      .setRequired(false)
      .addChoices(
        ...helpCategoryConfigs.map((config) => ({
...
const execute = async (
  ...
  const commandKey = interaction.options.getString("command") as HelpCommandKey | null;
  const categoryKey = interaction.options.getString("category") as HelpCategoryKey | null;
