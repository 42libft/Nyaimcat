...
import {
  buildFollowUpModalId,
  FOLLOW_UP_MODAL_DETAILS_INPUT_ID,
  FOLLOW_UP_MODAL_SUMMARY_INPUT_ID,
  FOLLOW_UP_MODAL_TITLE_INPUT_ID,
  parseFollowUpButtonId,
  parseFollowUpModalId,
} from "../../codex/followUp";
import type { AuditLogger } from "../auditLogger";
import { PRIORITY_LABELS, MAX_FILENAME_LENGTH } from "../commands/task";
import {
  findRecordedRunById,
  loadRecordedRunFromPath,
  type RecordedRunWithPath,
} from "../../codex/history";
...
type RunContext = {
  ...
  recorded?: RecordedRunWithPath | null;
};

const wrapCodeBlock = (value: string) => {
...
const buildFilename = (title: string) => {
  ...
  const slug =
    normalized.length > 0 ? normalized.slice(0, MAX_FILENAME_LENGTH) : "task";

  return `${timestamp}-${slug}.md`;
};

const summarizeInstructions = (value: string) => {
...
const normalizePriorityLabel = (priority: TaskPriority, label?: string | null) => {
...
const buildFallbackTask = (
  ...
  const defaultPriority = (recorded?.run.task.priority ?? "normal") as TaskPriority;
...
const resolveTaskPriority = (task: TaskFile): TaskPriority => {
...
export class CodexFollowUpManager {
  ...
  async handleButton(interaction: ButtonInteraction): Promise<boolean> {
    const payload = parseFollowUpButtonId(interaction.customId);
    if (!payload) {
      return false;
    }
    ...
    const titleInput = new TextInputBuilder()
      .setCustomId(FOLLOW_UP_MODAL_TITLE_INPUT_ID)
      .setLabel("件名 (任意)")
    ...
    const summaryInput = new TextInputBuilder()
      .setCustomId(FOLLOW_UP_MODAL_SUMMARY_INPUT_ID)
      .setLabel("概要 (任意)")
      .setStyle(TextInputStyle.Short)
      .setRequired(false)
    ...
    const detailsInput = new TextInputBuilder()
      .setCustomId(FOLLOW_UP_MODAL_DETAILS_INPUT_ID)
    ...
    const modal = new ModalBuilder()
      .setCustomId(buildFollowUpModalId(runId, interaction.user.id))
  ...
  async handleModalSubmit(
    ...
    const payload = parseFollowUpModalId(interaction.customId);
    if (!payload) {
      return false;
    }
    ...
    const titleValue =
      interaction.fields.getTextInputValue(FOLLOW_UP_MODAL_TITLE_INPUT_ID) ?? "";
    const summaryValue =
      interaction.fields.getTextInputValue(FOLLOW_UP_MODAL_SUMMARY_INPUT_ID) ?? "";
    const detailsValue =
      interaction.fields.getTextInputValue(FOLLOW_UP_MODAL_DETAILS_INPUT_ID) ?? "";
    ...
    try {
      ...
      const title =
        normalizeOptional(titleValue) ??
        `フォローアップ: ${context.task.metadata.title}`;
      const summary =
        normalizeOptional(summaryValue) ??
      ...
      const startResult = await codexWorkManager.startWork({
        filename: file.filename,
        notifyChannelId: interaction.channelId,
      });

      const prompt = buildPromptForTask(startResult.task).trim();
      const preview =
      ...
      logger.error("Codex フォローアップ処理でエラーが発生しました", {
        runId,
        userId: interaction.user.id,
        error: message,
      });
  ...
  private async resolveRunContext(runId: string): Promise<RunContext | null> {
    const stored = codexWorkManager.getRecentRunResult(runId);
    ...
    const queueItem = codexWorkManager.getQueueItemByRunId(runId);
    const historyPath = queueItem?.result?.historyPath ?? null;

    let recorded: RecordedRunWithPath | null = null;
    if (historyPath) {
      recorded = await loadRecordedRunFromPath(historyPath);
    }

    if (!recorded) {
      recorded = await findRecordedRunById(runId);
    }
    ...
    const filename =
      queueItem?.filename ?? recorded?.run.task.filename ?? `${runId}.md`;

    try {
      task = await readTaskFile(filename);
    } catch {
      /* noop */
    }
  ...
  private async createFollowUpTaskFile(args: {
    runId: string;
    interaction: ModalSubmitInteraction;
    context: RunContext;
    title: string;
    ...
    const priorityLabel = normalizePriorityLabel(
      priority,
      context.task.metadata.priority_label
    );
    ...
    const lines = [
      "---",
      dumpYaml(frontMatter, { lineWidth: 120 }).trimEnd(),
      "---",
      "",
      "## 概要",
      "",
