...
const truncateToLength = (value: string, limit: number) => {
...
const formatLogSnippet = (log: string, limit: number) => {
  if (!log.trim()) {
    return null;
  }
...
const formatRetryDetails = (retry: RetryLike) => {
...
const determineStatus = (result: CodexRunnerResult) => {
  if (result.timedOut) {
    return {
      label: "⏱ タイムアウト",
      color: WARNING_COLOR,
...
const describeCancellationStage = (queueItem: CodexQueueItem | null | undefined) => {
  if (!queueItem) {
    return "キャンセル済み";
  }
...
const chunkText = (value: string, size: number): string[] => {
  if (size <= 0) {
    return [value];
  }
...
const buildLogAttachments = (result: CodexRunnerResult): RawFile[] => {
...
export const buildRunNotification = (
  ...
  const stdoutLimit =
    typeof options.stdoutLimit === "number"
      ? options.stdoutLimit
      : (() => {
          const parsed = Number.parseInt(
            process.env.CODEX_DISCORD_NOTIFY_STDOUT_LIMIT ?? "",
            10
          );
          return Number.isFinite(parsed) ? parsed : DEFAULT_STDOUT_LIMIT;
        })();
  const stderrLimit =
    typeof options.stderrLimit === "number"
      ? options.stderrLimit
      : (() => {
          const parsed = Number.parseInt(
            process.env.CODEX_DISCORD_NOTIFY_STDERR_LIMIT ?? "",
            10
          );
  ...
  const historyPath = (() => {
    ...
    try {
      const relative = path.relative(REPO_ROOT, result.historyPath);
...
export const notifyRunResult = async (
  ...
  if (!actions) {
    try {
      ...
      logger.warn("DiscordActions の初期化に失敗したため通知をスキップします", {
        runId: result.runId,
...
const buildFailureTimeline = (queueItem?: CodexQueueItem | null) => {
  if (!queueItem) {
    return null;
  }
...
export const notifyRunFailure = async (
  ...
  if (!actions) {
    try {
      ...
      logger.warn("DiscordActions の初期化に失敗したため失敗通知をスキップします", {
        queueId: context.queueId,
        filename: context.task.filename,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  if (context.failureRecordPath) {
    const relative = (() => {
      try {
        const rel = path.relative(REPO_ROOT, context.failureRecordPath);
...
export const notifyRunCancellation = async (
  ...
  if (!actions) {
    try {
      ...
      logger.warn("DiscordActions の初期化に失敗したためキャンセル通知をスキップします", {
        queueId: context.queueId,
        task: context.task.filename,
        error: error instanceof Error ? error.message : String(error),
      });
