...
const historyReader_1 = require("../health/historyReader");
...
`;
const reportSkippedFiles = (skipped) => {
    if (skipped.length === 0) {
        return;
    }
    console.warn(`\u26a0\ufe0f ${skipped.length} 件のヘルス履歴ファイルを読み込めなかったため、集計対象から除外しました。`);
    const preview = skipped.slice(0, 5);
    for (const item of preview) {
        const relativePath = path_1.default.relative(process.cwd(), item.filePath);
...
const levelPriority = (level) => level === "error" ? 0 : 1;
...
const levelLabel = (level) => level === "error" ? "エラー" : "警告";
...
const formatDetails = (details) => {
    ...
    const lines = entries.slice(0, 5).map(([key, value]) => `${key}: ${formatDetailValue(value)}`);
    if (entries.length > 5) {
        lines.push(`…他 ${entries.length - 5} 件`);
    }
    return lines.join(" / ");
};
const aggregateHistory = (records) => {
    const issueMap = new Map();
    const timeline = [];
    for (const entry of records) {
        const snapshot = entry.record.type === "report"
            ? entry.record.issue
            : entry.record.issue;
        ...
        if (!aggregate) {
            aggregate = {
                ...
                active: true,
                lastEvent: entry.record.type === "report" ? entry.record.change : "resolved",
                reportCount: 0,
                createdCount: 0,
                updateCount: 0,
                resolveCount: 0,
        ...
        const timelineEntry = {
            ...
            change: entry.record.type === "report"
                ? entry.record.change
                : "resolved",
            recordedAt,
            filePath: entry.filePath,
            issue: snapshot,
            previous: entry.record.type === "report" ? entry.record.previous : null,
        };
...
const printIssueSummary = (issue) => {
    ...
    const status = issue.active
        ...
        : issue.resolveCount > 0
            ...
            : "状態不明";
    const lastEventLabel = issue.lastEvent === "resolved"
        ? "解消"
        : issue.lastEvent === "created"
            ? "新規検知"
            : "内容更新";
    const fileLabel = issue.lastFilePath
        ? path_1.default.relative(process.cwd(), issue.lastFilePath)
        : "-";
    ...
    console.log(`  レポート数  : ${issue.reportCount} (新規 ${issue.createdCount} / 更新 ${issue.updateCount})`);
    console.log(`  解消回数    : ${issue.resolveCount}`);
    console.log(`  最終ファイル: ${fileLabel}`);
    console.log(`  メッセージ  : ${issue.message}`);
};
const runSummary = async () => {
    const { records, skipped } = await (0, historyReader_1.loadHealthHistoryRecords)();
    ...
    console.log(`ヘルスチェック履歴: ${aggregation.issues.length} 件の issue を集計しました。`);
    console.log("----------------------------------------");
    ...
    const resolvedCount = aggregation.issues.length - activeCount;
    console.log(`内訳: 継続中 ${activeCount} 件 / 解消済み ${resolvedCount} 件`);
};
const printUpdateDifferences = (entry) => {
    if (entry.kind !== "report" || entry.change !== "updated" || !entry.previous) {
        return;
    }
    const changes = [];
    if (entry.previous.level !== entry.issue.level) {
        changes.push(`レベル: ${levelLabel(entry.previous.level)} → ${levelLabel(entry.issue.level)}`);
    }
...
const printTimelineEntry = (entry) => {
    const recorded = formatTimestamp(entry.recordedAt);
    const label = eventLabel(entry);
    const fileLabel = path_1.default.relative(process.cwd(), entry.filePath);
...
const parseLimit = (args, defaultValue) => {
...
const runTimeline = async (args) => {
    ...
    for (const entry of aggregation.timeline.slice(0, limit)) {
        ...
        console.log(`    履歴ファイル: ${path_1.default.relative(process.cwd(), entry.filePath)}`);
        console.log("");
    }
};
const main = async () => {
...
//# sourceMappingURL=healthHistory.js.map